...
class Docstring():
    def __init__(self, source, output, config) -> None:
        ...
        self.relevant_object_keys = [
            f" As {x}" for x in self.relevant_objects.keys()]
        self.mermaid_screenflow = self._create_mermaid_screenflow()
    ...
    def _extract_screen_content_to_markdown(self, screen_objects) -> None:
        ...
        mermaid_screens = list(
            filter(lambda x: screen_objects[0] in x, self.mermaid_screenflow))

        if mermaid_screens:
            mermaid_screens = [self.config["MermaidPrefix"]] + \
                ["graph LR"] + mermaid_screens + \
                [self.config["MermaidSuffix"]]

        for line in mermaid_screens:
        ...
        if self.config["ScreenFlow"]["ShowVariables"]:
            ...
            global_variables = re.findall(
                r"Set\((.[^,]*)", str(screen_objects[1]))
            global_variables = list(set(global_variables))
            global_variables = self._cleanUpList(global_variables)

            self.md_file.new_header(level=3, title="Variables")
            self.md_file.new_line(
                "Following variables have been created / or updated on this screen")

            global_variables_links = list(
                map(lambda x: "["+x+"]" + "(#" + str.lower(x) + ")", global_variables))

            self.md_file.new_list(global_variables_links)

        # add collects to screen if enabled in config
        if self.config["ScreenFlow"]["ShowCollects"]:
            # find variable with regex pattern
            global_collects = re.findall(
                r"Collect\((.[^,]*)", str(screen_objects[1]))
            global_collects = list(set(global_collects))
            global_collects = self._cleanUpList(global_collects)
            self.md_file.new_header(level=3, title="Collects")
            self.md_file.new_line(
                "Following collects have been created, updated or cleared on this screen")

            global_collects_links = list(
                map(lambda x: "["+x+"]" + "(#" + str.lower(x) + ")", global_collects))

            self.md_file.new_list(global_collects_links)

        # add flows to screen if enabled in config
        if self.config["ScreenFlow"]["ShowFlows"]:
            # find variable with regex pattern
            flows = re.findall(
                r"(\w*).Run", str(screen_objects[1]))
            flows = list(set(flows))
            flows = self._cleanUpList(flows)

            self.md_file.new_header(level=3, title="Flows")
            self.md_file.new_line(
                "Following flows have used on this screen")

            flows_links = list(
                map(lambda x: "["+x+"]" + "(#" + str.lower(x) + ")", flows))

            self.md_file.new_list(flows_links)
                ...
                for sub_dict_key, sub_dict_item in value.items():
                    if sub_dict_key in self.relevant_objects.get(rel_obj_key):
                        if sub_dict_key == "OnVisible":
                            screen_docstring = self._extract_parts_from_propperty(
                                sub_dict_item)
    ...
    def _create_chapter_connections(self):
        ...
        if len(connections) > 0:
            for connection in connections.items():
                ...
                self.md_file.new_line("")
                self.md_file.new_list(
                    [x for x in connection[1]["dataSources"]])
                self.md_file.new_line("")
    ...
    def _create_chapter_screens(self):
        ...
        for scr_flow in self.mermaid_screenflow:
    ...
    def _create_global_variables(self):
        ...
        for file in self._get_screen_files():
            ...
            global_variables_on_screen : list = self._cleanUpList(global_variables_on_screen)
        ...
        for file in self._get_screen_files():
            ...
            for variable in all_global_variables:
                used_variables_on_screen = used_variables_on_screen + \
                    re.findall(f"(?<!Set\(){variable}", str(screen_objects[1]))
    ...
    def _create_global_collects(self):
        ...
        for file in self._get_screen_files():
            ...
            all_collects_on_screen = re.findall(
                r"Collect\((.[^,]*)", str(screen_objects[1]))

            all_collects_on_screen = self._cleanUpList(all_collects_on_screen)

            s= str(screen_objects[1])
            r= re.compile(
                r"Collect\((?P<collect>.[^,]*)|ClearCollect\((?P<clearCollect>.[^,]*)")
            groups = [m.groupdict() for m in r.finditer(s)]

            collects_on_screen = [d["collect"]
                                  for d in groups if d['collect']]
            collects_on_screen = self._cleanUpList(collects_on_screen)

            clearCollects_on_screen = [d["clearCollect"]
                                       for d in groups if d['clearCollect']]
            clearCollects_on_screen = self._cleanUpList(
                clearCollects_on_screen)

            all_collects_on_screen = list(set(all_collects_on_screen))
            collects_on_screen = list(set(collects_on_screen))
            clearCollects_on_screen = list(set(clearCollects_on_screen))

            screen_all_collects[screen_objects[0]
                                ] = all_collects_on_screen

            screen_set_collects[screen_objects[0]
                                ] = collects_on_screen

            screen_clear_collects[screen_objects[0]
                                  ] = clearCollects_on_screen

        # get all collects
        all_collects = [
            item for sublist in screen_all_collects.values() for item in sublist]
        all_collects = list(set(all_collects))

        for file in self._get_screen_files():
            screen_objects = self.parser.get_screen_objects(file)
            used_collects_on_screen = []

            # check if collects are used on this screen
            for collect in all_collects:
                used_collects_on_screen = used_collects_on_screen + \
                    re.findall(
                        f"(?<!Collect\(){collect}", str(screen_objects[1]))
                used_collects_on_screen = self._cleanUpList(
                    used_collects_on_screen)


            # remove duplicates
            used_collects_on_screen = list(set(used_collects_on_screen))
            screen_use_collects[screen_objects[0]] = used_collects_on_screen

        # create mermaid for each variable (where set, where used and cleared)
        ...
        for variable in all_collects:
            variable_mermaid = [self.config["MermaidPrefix"]]
            variable_mermaid.append("graph LR")

            for key, value in screen_set_collects.items():
                if variable in value:
                    variable_mermaid.append(
                        "Collect" + "".join(key.split()) + r"(" + key + r")" +
                        "-- collect -->" +
                        "".join(variable.split()) + r"[/" + variable + r"/]"
                    )

            for key, value in screen_use_collects.items():
                if variable in value:
                    variable_mermaid.append(
                        "".join(variable.split()) + r"[/" + variable + r"/]" +
                        "-. use .->" +
            ...
            for key, value in screen_clear_collects.items():
                if variable in value:
                    variable_mermaid.append(
                        "".join(variable.split()) + r"[/" + variable + r"/]" +
                        "-. clear .->" +
                        "Clear" + "".join(key.split()) + r"(" + key + r")"
                    )
    ...
    def _create_global_flows(self):
        screen_use_flows = {}

        for file in self._get_screen_files():
            # check on which screen variables are set
            screen_objects = self.parser.get_screen_objects(file)
            global_flows_on_screen = re.findall(
                r"(\w*).Run", str(screen_objects[1]))
            global_flows_on_screen = list(set(global_flows_on_screen))
            screen_use_flows[screen_objects[0]
                                 ] = global_flows_on_screen

            global_flows_on_screen = self._cleanUpList(global_flows_on_screen)

        # get all set global flows
        all_global_flows = [
            item for sublist in screen_use_flows.values() for item in sublist]
        all_global_flows = list(set(all_global_flows))

        for file in self._get_screen_files():
            screen_objects = self.parser.get_screen_objects(file)
            used_flows_on_screen = []

            # check if variables are used on this screen
            for variable in all_global_flows:
                used_flows_on_screen = used_flows_on_screen + \
                    re.findall(f"{variable}(?<!.Run)", str(screen_objects[1]))

            # remove duplicates
            used_flows_on_screen = list(set(used_flows_on_screen))
            screen_use_flows[screen_objects[0]] = used_flows_on_screen

        # create mermaid for each variable (where set and where used)
        self.md_file.new_header(level=1, title="Global Flows")
        self.md_file.new_line(
        ...
        for variable in all_global_flows:
            ...
            for key, value in screen_use_flows.items():
                if variable in value:
                    variable_mermaid.append(
                        "".join(variable.split()) + r"[/" + variable + r"/]" +
                        "-. executed_from .->" +
                        "executed_from" + "".join(key.split()) + r"(" + key + r")"
                    )
    ...
    def create_documentation(self, format=None):
        ...
        self.md_file = MdUtils(file_name=output_file,
                               title='Power App Documentation')
        ...
        for chapter in self.config["DocumentStructure"]:
            ...
            elif chapter == "GlobalFlows":
                self._create_global_flows()
        # write toc + file
